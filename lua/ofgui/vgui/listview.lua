...
function PANEL:Init()
	...
	self:SetHeaderHeight(16 * OFGUI.ScreenScale)
	self:SetDataHeight(17 * OFGUI.ScreenScale)
...
function PANEL:Paint(w, h)
	draw.RoundedBox(6 * OFGUI.ScreenScale, 0, 0, w - 19 * OFGUI.ScreenScale, h, OFGUI.BGColor)
end
...
function PANEL:AddColumn(strName, iPosition)
	...
	pColumn:SetZPos(10 * OFGUI.ScreenScale)
...
function PANEL:PerformLayout()
	...
	if IsValid(self.VBar) then
		self.VBar:SetPos(self:GetWide() - 16 * OFGUI.ScreenScale, 0)
		self.VBar:SetSize(16 * OFGUI.ScreenScale, self:GetTall())
		self.VBar:SetUp(self.VBar:GetTall() - self:GetHeaderHeight(), self.pnlCanvas:GetTall())
		yPos = self.VBar:GetOffset()

		if self.VBar.Enabled then
			wide = wide - 19 * OFGUI.ScreenScale
		end
	end
...
function PANEL:DataLayout()
	...
	for k, line in ipairs(self.Sorted) do
		line:SetPos(1, y)
		line:SetSize(self.pnlCanvas:GetWide() - 2 * OFGUI.ScreenScale, h)
