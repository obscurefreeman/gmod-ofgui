...
function PANEL:Init()
	...
	self.Header.Paint = function(pan, w, h)
		...
		draw.RoundedBoxEx(6 * OFGUI.ScreenScale, 0, 0, w - (id < #self:GetParent().Columns and 1 or 0), h, pan.Color, not (id > 1), not (id < #self:GetParent().Columns), false, false)
	end
	...
	self:SetMinWidth(10 * OFGUI.ScreenScale)
	self:SetMaxWidth(19200 * OFGUI.ScreenScale)
end
...
function PANEL:PerformLayout()
	...
	self.DraggerBar:SetWide(4 * OFGUI.ScreenScale)
